...
public class MicrosoftTranslate implements Itranslate {
    ...
    @Override
    public void makeRequest(final String text, final String langFrom, final String langTo, final TextView textView) {
        ...
        final StringRequest stringRequest = new StringRequest(Request.Method.GET, finalUrl, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                lastResponse = response;
                final String res = response.substring(response.indexOf('>') + 1, response.lastIndexOf('<'));
                textView.setText(res);
                DataBase.save(context, text, res, langFrom + "-" + langTo);
            }
        }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
